...
export default function FollowButton({ profileId }) {
    ...
    return (
        <>
            {seguido ? (
                <button className='buttonFollowsss' onClick={unfollow}><PersonAddDisabled className='buttonIcon'/>Dejar de seguir</button>
            ) : (
                <button className='buttonFollowsss' onClick={follow}><PersonAddAlt1 className='buttonIcon' />Seguir</button>
            )}
        </>
    );
}